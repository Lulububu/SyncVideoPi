...
void OMXComponent::GetState(OMX_STATETYPE *pState)
{
	OMX_ERRORTYPE err = OMX_GetState(m_handle, pState);
...
void OMXComponent::UseEGLImage(OMX_BUFFERHEADERTYPE **ppBufferHdr, OMX_U32 nPortIndex, OMX_PTR pAppPrivate, void *eglImage)
{
	OMX_ERRORTYPE err = OMX_UseEGLImage(m_handle, ppBufferHdr, nPortIndex, pAppPrivate, eglImage);

	if (err != OMX_ErrorNone)
	{
		throw runtime_error("OMX_UseEGLImage failed");
	}
}
...
static void add_milliseconds(uint32_t *puSec, uint32_t *pu32NanoSec, uint32_t u32MillisecondsToAdd)
{
   *puSec  += u32MillisecondsToAdd / 1000;
   *pu32NanoSec += (u32MillisecondsToAdd % 1000) * 1000000;

   // if we've overflowed the nanosecond boundary
   if (*pu32NanoSec > 1000000000)
   {
      *puSec  += 1;
      *pu32NanoSec -= 1000000000;
   }
}
...
IEventSPtr OMXComponent::WaitForAnything(unsigned int uTimeoutMs)
{
	...
	while (!bFound)
	{
		...
		{
			OMXEvent *pEvent = new OMXEvent(m_lstEvents.front());
...
IOMXComponentSPtr OMXComponent::GetInstance(ILogger *pLogger, ILocker *pLocker, IClock *pClock)
{
	return IOMXComponentSPtr(new OMXComponent(pLogger, pLocker, pClock), OMXComponent::deleter());
}

OMXComponent::OMXComponent(ILogger *pLogger, ILocker *pLocker, IClock *pClock) :
m_pLogger(pLogger),
m_pLocker(pLocker),
m_pClock(pClock)
{
